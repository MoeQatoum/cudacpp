  ...
  #include "rtc/Kernel.h"
  #include "rtc/Program.h"
...
int testinvokeRTC(int size) {
  ...
  prog.compile({rtc::options::GpuArchitecture(dev.properties()), rtc::options::CPPLang(rtc::options::CPP_x14)});
  ...
  auto check = std::accumulate(ptr, ptr + 32 * 10, true, [](auto res, auto num) { return res && num == 23.1f; });
