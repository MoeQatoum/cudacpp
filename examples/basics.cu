...
__global__ void
  addKernel(cudacpp::DeviceVector<T> c, const cudacpp::DeviceVector<T> a, const cudacpp::DeviceVector<T> b) {
...
__device__ __inline__ int my1DimIndex() {
  ...
  int threadId = blockId * (blockDim.x * blockDim.y * blockDim.z) + threadIdx.x + threadIdx.y * blockDim.x
...
struct OpPlus : public Op<int> {
  ...
  __device__ __host__ int operator()(int t1, int t2) const override { return t1 + t2 + *_i; }
};

/*
template<typename T, typename INFO>
__global__ void addKernelOp(DevicePtr<T> c, DevicePtr<T const> a, DevicePtr<T
const> b, INFO info, DevicePtr<OpPlus> op)
{
        //int myidx = my1DimIndex();
        auto myIdx = info.index();
        if (info.inRange(myIdx)) {
...
template<int DIM_BLOCKS, int DIM_THREADS, typename T>
__global__ void addKernelOp(cudacpp::DeviceVector<T>       c,
                            const cudacpp::DeviceVector<T> a,
                            const cudacpp::DeviceVector<T> b,
                            DevicePtr<Op<T>>               op) {
  auto idx = cudacpp::Index<1>::create<DIM_BLOCKS, DIM_THREADS>();
  if (idx.inRange(c.size())) {
...
template<int DIM_BLOCKS, int DIM_THREADS, typename T>
__global__ void
  addKernelLambda(cudacpp::DeviceVector<T> c, const cudacpp::DeviceVector<T> a, const cudacpp::DeviceVector<T> b) {
  auto idx = cudacpp::Index<1>::create<DIM_BLOCKS, DIM_THREADS>();
  if (idx.inRange(c.size())) {
...
int basics(int size, const int* a, const int* b, int* c) {
  ...
  //	addKernelOp<<<1, size>>>(MakeDevicePtr(cDev), pa, MakeDevicePtr(b),
  // grid.info(), (DevicePtr<OpPlus>)op);

  // addKernel2<<<grid.blocks, grid.threads>>>(MakeDevicePtr(cDev), pa,
  // MakeDevicePtr(b), grid.info());
...
cudaError_t addWithCuda(int* c, const int* a, const int* b, unsigned int size) {
  ...
  cudaStatus = cudacpp::CopyElements(dev_a, a, size); // cudaMemcpy(dev_a, a, size * sizeof(int),
  ...
  // addKernel<grid.DimBlocks, grid.DimThreads, int><<<grid.blocks,
  // grid.threads>>>(dev_c, vec_a, dev_b); addKernelOp<grid.DimBlocks,
  // grid.DimThreads, int> << <grid.blocks, grid.threads >> >(dev_c, vec_a,
  // dev_b, (DevicePtr<OpPlus>)op);
  ...
  cudaStatus = cudacpp::CopyElements(c, dev_c, size); // cudaMemcpy(c, dev_c, size * sizeof(int),
