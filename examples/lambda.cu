...
__device__ __inline__ void SetVal(int &i, int val) { i = val; }
...
__global__ void applyKernelOp(cudacpp::DeviceVector<int> c,
                              cudacpp::DeviceVector<int> a,
                              cudacpp::DeviceVector<int> b, OP op) {
  auto idx = threadIdx.x;
  c[idx] = op.op()(a[idx], b[idx]);
}

__global__ void applyKernelDirect(cudacpp::DeviceVector<int> c,
                                  cudacpp::DeviceVector<int> a,
                                  cudacpp::DeviceVector<int> b, int val) {
  auto idx = threadIdx.x;
