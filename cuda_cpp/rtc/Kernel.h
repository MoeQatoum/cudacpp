...
namespace cudacpp {
  namespace rtc {
     ...
     *                                           // The parameters can include a type - for typename|class, or
     * integral_constant for integrals <EXAMPLE> Assume in the CUDA code you have
     *
    ...
    namespace detail {
      ...
      struct NameExtractor<std::integral_constant<T, t>> {
        static std::string extract() { return std::to_string(t); }
      };
    } // namespace detail
    ...
    template<typename... ARGS>
    inline CUresult
      Kernel::launch(const Stream& stream, const Grid<3, 3, 3>& grid, unsigned int sharedMem, const ARGS&... args) {
