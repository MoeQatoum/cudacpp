...
namespace cudacpp {
  ...
  struct Index<1> : public Size<1> {
    ...
    template<>
    __host__ __device__ __inline__ Index create<0, 1>() {
      return {threadIdx.x};
    }

    template<>
    __host__ __device__ __inline__ Index create<1, 1>() {
      return {(blockIdx.x * blockDim.x) + threadIdx.x};
    }

    template<>
    __host__ __device__ __inline__ Index create<2, 1>() {
      int blockId = blockIdx.y * gridDim.x + blockIdx.x;
      return {blockId * blockDim.x + threadIdx.x};
    }

    template<>
    __host__ __device__ __inline__ Index create<3, 1>() {
  ...
  template<>
  struct Index<2> : public Size<2> {
    using Size<2>::Size;

    template<int DIM_BLOCKS, int DIM_THREADS>
    __device__ __inline__ Index create();

    template<>
    __device__ __inline__ Index create<0, 2>() {
      return {threadIdx.x, threadIdx.y};
    }

    template<>
    __device__ __inline__ Index create<2, 2>() {
